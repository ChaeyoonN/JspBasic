...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getMethod().equals("POST")) {//한글 깨짐 방지(get방식은 필요없음)
			request.setCharacterEncoding("UTF-8");
		}
		
		String uri= request.getRequestURI(); //uri 정제
		...
		case "write":
			...
			response.sendRedirect("board/board_write.jsp"); //index.html기준으로 한 상대경로
			break;
...
//			response.sendRedirect("board/board_list.jsp");
			...
			//디스패쳐객체의 path에는 절대경로 적지 X(ContextPath X)
			
			dp.forward(request, response); //디스패쳐객체가 이 두 객체를 해당경로로 운반한다.
